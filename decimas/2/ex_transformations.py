...
def returnpoints(theta,radius,center_x,center_y):
    return np.array([radius*np.cos(theta)+center_x, radius*np.sin(theta)+center_y, 0.0,
...
def createCircle():
    ...
    for i in range(0,360):
        vertexData = np.concatenate((vertexData,returnpoints(i*(np.pi/180),1.0,0.5,0.3)))
    # Defining connections among vertices
    # We have a triangle every 3 indices specified
    indices = np.array([
        0, 1, 2
